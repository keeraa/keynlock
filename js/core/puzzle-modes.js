...
(() => {
  const BASE_MODES = new Set(['classic', 'sequence', 'special', 'keyprofile', 'wharf', 'springtumblers', 'symbolpins']);
  ...
  function syncMode(mode, options = {}) {
    if (BASE_MODES.has(mode)) activate(options.profile || 'base-plates', {...options,slotted:mode==='keyprofile'||mode==='wharf'||mode==='springtumblers'||mode==='symbolpins'});
    else deactivate();
  }
