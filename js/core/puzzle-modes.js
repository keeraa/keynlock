...
(() => {
  const BASE_MODES = new Set(['classic', 'sequence', 'special', 'hillsfar', 'wharf', 'watchmen', 'alphaprotocol']);
  ...
  function syncMode(mode, options = {}) {
    if (BASE_MODES.has(mode)) activate(options.profile || 'base-plates', {...options,slotted:mode==='hillsfar'||mode==='wharf'||mode==='watchmen'||mode==='alphaprotocol'});
    else deactivate();
  }
