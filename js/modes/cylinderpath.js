(function(){
  ...
  // game module: cylinderpath") into a fully native mode: the tracking math
  ...
  function startCylinderpathRound(){
    solved=false;
    ...
    kcdStress=0;
    kcdTurning=false;
    kcdTolerance=diffStep(.11,.082,.06,'cylinderpath');
    kcdTargetRot=diffStep(180,220,260,'cylinderpath');
    kcdRandomPointer();
  ...
  function kcdPointerMove(x,y){
    if(mode!=='cylinderpath' || solved) return;
  ...
  function kcdBreakPick(){
    ...
    damagePick({
      ...
      renderState:renderCylinderpath,
  ...
  function kcdTick(dt){
    if(mode!=='cylinderpath' || solved || !kcdTurning) return;
    const d=kcdDist();
  ...
  PuzzleModes.register({
    id:'cylinderpath', start:startCylinderpathRound, render:renderCylinderpath,
    tick:({dt})=>kcdTick(Math.min(.04,dt/1000)),
    objective:()=>GameCatalog.get('cylinderpath')?.objective,
    restartMessage:'Новая попытка: «Ход цилиндра»',
    input:{
    ...
    attemptOpen:tryOpenCylinderpath
  });
})();
