(function(){
  ...
  function renderDeduction(){
    ...
    kdVals.forEach((v,i)=>{
      ...
      t.innerHTML=`<div class="kdNum">${v}</div><div class="kdBar" style="--h:${v}"><img class="kdPinImg" src="${pinSkin}" alt=""></div><div class="kdControls"><button class="kdMini" data-d="-1" type="button" aria-label="Опустить штифт ${i+1}">−</button><button class="kdMini" data-d="1" type="button" aria-label="Поднять штифт ${i+1}">+</button></div>`;
  ...
  function startDeductionRound(){
    ...
    $kdFeedback.textContent='Нажми «Проверить ключ». Замок укажет первый неверный штифт: «выше» — нажми +, «ниже» — −. Каждая третья неверная проверка ломает отмычку.';
    renderDeduction();
  }
  ...
  function checkDeduction(){
    ...
    kdSelected=wrong;
    $kdFeedback.textContent=`Штифт ${wrong+1}: ${low?'нужно выше — нажми +':'нужно ниже — нажми −'}. ${wrong>0?'Все штифты слева уже верны. ':''}Затем снова проверь ключ.`;
    kdLogs.push(`${wrong+1}: ${low?'↑':'↓'}`);kdFailures++;
    SFX.wrongLock();
    if(kdFailures%3===0){
      damagePick({
        resetProgress:()=>{kdVals=Array.from({length:kdToothCount},()=>2);kdSelected=0;$kdFeedback.textContent='Отмычка сломалась, высоты сброшены на 2. Нажми «Проверить ключ», чтобы получить новую подсказку.';},
  ...
  function tryOpenDeduction(){
    ...
    solved=true;SFX.open();renderDeduction();
