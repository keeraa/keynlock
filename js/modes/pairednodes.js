(function(){
  // ===== PAIREDNODES (pairednodes: pairednodes) =====
  ...
  // "Portable game module: pairednodes") into a fully native mode, wired
  ...
  function m2RegenerateBoard(){
    ...
    m2Matched=new Set();
    m2Lock=false;
    m2TimeMax=window.KeynlockCampaign?.training('pairednodes')?75:diffStep(48,40,32);
    m2TimeLeft=m2TimeMax;
    const symbols=window.KeynlockCampaign?.training('pairednodes')?M2_SYMBOLS.slice(0,4):M2_SYMBOLS;
  ...
  function startPairednodesRound(){
    solved=false;
  ...
  function renderPairednodes(){
    ...
    renderPairednodesHud();
  }

  function renderPairednodesHud(){
    setGlobalTimer(mode==='pairednodes' && !solved, m2TimeLeft, m2TimeMax, 'ТАЙМЕР');
  }
  ...
  function m2Click(i){
    ...
    if(m2Nodes[i]===m2Nodes[m2Sel]){
      ...
      SFX.move();
      renderPairednodes();
      if(m2Matched.size===m2Nodes.length) tryOpenPairednodes();
      return;
    }
    m2Lock=true;
    SFX.wrongLock();
    damagePick({
      renderState:renderPairednodes,
  ...
  function m2Tick(dt){
    if(mode!=='pairednodes' || solved) return;
  ...
  PuzzleModes.register({
    id:'pairednodes',
    start:startPairednodesRound,
    render:renderPairednodes,
    tick:({dt})=>m2Tick(Math.min(.05,dt/1000)),
    syncHud:renderPairednodesHud,
    objective:()=>GameCatalog.get('pairednodes')?.objective,
    ...
    attemptOpen:tryOpenPairednodes
  });
})();
