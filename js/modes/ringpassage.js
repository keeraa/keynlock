...
function startRingpassageRound(){
  ...
  updateEconomyUI(); renderRingpassage();
}
...
function tryOpenRingpassage(){
  ...
  solved=true;
  $lock.classList.add('win'); SFX.open(); renderRingpassage();
  setTimeout(()=>celebrate(),420);
}
function hackTick(dt){
  if(mode!=='ringpassage'||solved) return;
  const now=performance.now();
...
$hackCore?.addEventListener('click',()=>GameActions.attemptOpen({modeId:'ringpassage',source:'puzzle-control'}));
PuzzleModes.register({id:'ringpassage',start:startRingpassageRound,render:renderRingpassage,tick:({dt})=>hackTick(Math.min(.05,dt/1000)),objective:()=>GameCatalog.get('ringpassage')?.objective,restartMessage:'Новая попытка: «Кольцевой проход»',input:{horizontal:hackMove,vertical:delta=>delta<0?hackIn():hackOut()},actions:{primary:()=>GameActions.attemptOpen({modeId:'ringpassage',source:'keyboard'}),secondary:()=>GameActions.attemptOpen({modeId:'ringpassage',source:'keyboard'})},attemptOpen:tryOpenRingpassage});
})();
