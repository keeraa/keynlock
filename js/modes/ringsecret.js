(function(){
  ...
  // ringsecret") into a fully native mode: the angle/sweet-spot
  ...
  function startRingsecretRound(){
    ...
    tdsFailed=false;
    tdsStep=0;
    tdsTimeMax=diffStep(26,22,18,'ringsecret');
  ...
  function tdsUpdateHot(){
    const tolerance=diffStep(18,14,10,'ringsecret');
  ...
  function tdsPointerAngle(e){
    if(mode!=='ringsecret' || tdsFailed || tdsStep>=3) return;
  ...
  function tdsCommit(){
    ...
    if(tdsSelectedRing!==expected){
      SFX.wrongLock();
      damagePick({ renderState:renderRingsecret, surviveText:'Выбрано не то кольцо' });
      return;
    }
    if(!tdsHot){
      SFX.wrongLock();
      damagePick({ renderState:renderRingsecret, surviveText:'Промах мимо сладкой точки' });
      return;
    }
  ...
  function tdsTick(dt){
    if(mode!=='ringsecret' || solved || tdsFailed || tdsStep>=3) return;
  ...
  function renderRingsecret(){
    if(!$tdsLock) return;
    setGlobalTimer(mode==='ringsecret' && !tdsFailed, tdsTimeLeft, tdsTimeMax, 'ТАЙМЕР');
  ...
  function renderRingsecretHud(){
    setGlobalTimer(mode==='ringsecret'&&!tdsFailed,tdsTimeLeft,tdsTimeMax,'ТАЙМЕР');
  }

  function tryOpenRingsecret(){
    if(solved) return;
  ...
  PuzzleModes.register({
    id:'ringsecret', start:startRingsecretRound, render:renderRingsecret,
    tick:({dt})=>tdsTick(Math.min(.05,dt/1000)),
    syncHud:renderRingsecretHud,
    objective:()=>GameCatalog.get('ringsecret')?.objective,
    restartMessage:'Новая попытка: «Кольцевой секрет»',
    input:{
      horizontal:tdsMove,
      vertical:delta=>tdsSelectRing(tdsSelectedRing+(delta<0?-1:1))
    },
    actions:{primary:tdsCommit},
    attemptOpen:tryOpenRingsecret
  });
})();
