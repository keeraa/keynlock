(function(){
  ...
  function startAnRound(){
    ...
    brokenPicks=0;
    runReward=100;
    const anMax=diffStep(6,8,9,'signalbalance');
    const minDistance=diffStep(6,10,14,'signalbalance');
    let dist=0;
    do{
  ...
  PuzzleModes.register({
    id:'signalbalance', start:startAnRound, render:renderAn,
    objective:()=>GameCatalog.get('signalbalance')?.objective,
    restartMessage:'Новая попытка: «Баланс сигналов»',
