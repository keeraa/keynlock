(function(){
  ...
  // soundlatch") into a fully native mode: the elimination logic carries
  ...
  function startSoundlatchRound(){
    ...
    th12Tried=new Set();
    th12Failed=false;
    th12TimeMax=diffStep(28,22,17,'soundlatch');
    th12TimeLeft=th12TimeMax;
    th12HoldDuration=diffStep(.45,.63,.85,'soundlatch');
    th12LastHint='';
    th12KeyType=null;
  ...
  function th12Use(type,on){
    if(mode!=='soundlatch' || solved) return;
    if(!on){
  ...
  function th12Tick(dt){
    if(mode!=='soundlatch' || solved || th12Failed) return;
    if(th12Step<5){
  ...
  function renderSoundlatch(){
    if(!$th12Door) return;
    setGlobalTimer(mode==='soundlatch' && !th12Failed, th12TimeLeft, th12TimeMax, 'ТАЙМЕР');
    if($th12Stages){
  ...
  addEventListener('keydown',e=>{
    if(gameplayInputBlocked()||mode!=='soundlatch') return;
  ...
  PuzzleModes.register({
    id:'soundlatch', start:startSoundlatchRound, render:renderSoundlatch,
    tick:({dt})=>th12Tick(Math.min(.05,dt/1000)),
    syncHud:renderSoundlatch,
    objective:()=>GameCatalog.get('soundlatch')?.objective,
    restartMessage:'Новая попытка: «Звуковой зацеп»',
    input:{horizontal:()=>{},vertical:()=>{}},
    attemptOpen:tryOpenSoundlatch
  });
})();
