(function(){
  ...
  // "Portable game module: springtumblers") into a fully native mode: same
  ...
  function startSpringtumblersRound(){
    chooseGamePinSkin();
    solved=false;
    ...
    wmRegeneratePins();
    updateEconomyUI();
    renderSpringtumblers();
  }

  function renderSpringtumblers(){
    if(!$wmLock) return;
  ...
  function wmAdjust(dir){
    ...
    if((actual>0 && p.h>=WM_MAX-1) || (actual<0 && p.h<=WM_MIN+1)){
      SFX.wrongLock();
      damagePick({
        renderState:renderSpringtumblers,
  ...
  function wmTick(dt){
    if(mode!=='springtumblers' || solved) return;
  ...
  function tryOpenSpringtumblers(){
    if(solved) return;
  ...
  PuzzleModes.register({
    id:'springtumblers',
    start:startSpringtumblersRound,
    render:renderSpringtumblers,
    tick:({dt})=>wmTick(Math.min(.04,dt/1000)),
    resize:()=>{ wmApplyGeometry(); renderSpringtumblers(); },
    objective:()=>GameCatalog.get('springtumblers')?.objective,
    restartMessage:'Новая попытка: «Подпружиненные тумблеры»',
    input:{
      horizontal:wmMove,
      vertical:delta=>delta<0?wmRaise():wmLower()
    },
    actions:{primary:wmTryLock},
    attemptOpen:tryOpenSpringtumblers
  });
})();
