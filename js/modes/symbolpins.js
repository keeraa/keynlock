...
function startSymbolpinsRound(){
  ...
  apTimeMax=diffStep(30,26,22,'symbolpins'); apTimeLeft=apTimeMax;
  updateEconomyUI(); renderSymbolpins();
}
...
function apSet(){
  ...
  if(apSel!==expected){
    ...
    SFX.wrongLock();
    damagePick({ resetProgress:()=>{}, renderState:renderSymbolpins, surviveText:'Неверный символ' });
    return;
  }
  if(apReady(apSel)){
  ...
  SFX.wrongLock();
  damagePick({ resetProgress:()=>{}, renderState:renderSymbolpins, surviveText:'Щель не совпала' });
}
function tryOpenSymbolpins(){
  if(solved) return;
  if(apOrderStep<apOrder.length){
    SFX.wrongLock();
  ...
  solved=true;
  $lock.classList.add('win'); SFX.open(); renderSymbolpins();
  setTimeout(()=>celebrate(),420);
}
function apTick(dt){
  if(mode!=='symbolpins'||solved||apTimeLeft<=0) return;
  apTimeLeft=Math.max(0,apTimeLeft-dt);
  renderSymbolpins();
}
function renderSymbolpins(){
  if(!$apLock) return;
  setGlobalTimer(mode==='symbolpins' && apTimeLeft>0, apTimeLeft, apTimeMax, 'ТАЙМЕР');
  apPins.forEach((p,i)=>{
...
PuzzleModes.register({id:'symbolpins',start:startSymbolpinsRound,render:renderSymbolpins,tick:({dt})=>apTick(Math.min(.05,dt/1000)),syncHud:renderSymbolpins,objective:()=>GameCatalog.get('symbolpins')?.objective,restartMessage:'Новая попытка: «Символьные штифты»',input:{horizontal:dir=>apSelectPin(apSel+dir),vertical:delta=>apMovePin(delta<0?-1:1)},actions:{primary:apSet,secondary:()=>GameActions.attemptOpen({modeId:'symbolpins',source:'keyboard'})},attemptOpen:tryOpenSymbolpins});
})();
