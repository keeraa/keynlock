...
function bioZones(stage){
  ...
  for(let i=0;i<c.greenCount;i++) blocks.push({t:'green',w:c.greenW*(window.KeynlockCampaign?.balance?.('timingneedle')?1.35:1)});
...
function startTimingneedleRound(){
...
function tryOpenTimingneedle(){
  if(solved) return;
  toast('Останови стрелку на зелёной зоне все 5 этапов — дрон взломается сам');
}
function bioTick(dt){
  if(mode!=='timingneedle'||solved||!bioRunning) return;
...
PuzzleModes.register({id:'timingneedle',start:startTimingneedleRound,render:renderBio,tick:({dt})=>bioTick(Math.min(.04,dt/1000)),objective:()=>GameCatalog.get('timingneedle')?.objective,restartMessage:'Новая попытка: «Точная остановка»',input:{horizontal:()=>{},vertical:()=>{}},actions:{primary:bioStop},attemptOpen:tryOpenTimingneedle});
})();
