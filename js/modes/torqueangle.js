(function(){
  ...
  // torqueangle") into a fully native mode: the wear/stall math carries over
  ...
  // ringsecret earlier this session).

  const SF_MAX_TURN=88;

  function startTorqueangleRound(){
    solved=false;
    ...
    sfStall=0;
    sfFailed=false;
    sfSuccessTol=diffStep(12,8,5,'torqueangle');
    sfLastHint='';
  ...
  function sfPointerMove(e){
    if(mode!=='torqueangle' || solved || sfFailed || sfTorqueDir) return;
  ...
  function sfTorque(dir){
    if(mode!=='torqueangle' || solved || sfFailed) return;
    sfTorqueDir=dir;
  ...
  addEventListener('pointermove',e=>{ if(mode==='torqueangle') sfPointerMove(e); },{passive:true});
  ...
  addEventListener('keydown',e=>{
    if(gameplayInputBlocked()||mode!=='torqueangle') return;
  ...
  addEventListener('keyup',e=>{
    if(mode!=='torqueangle') return;
    if(e.code==='KeyA'||e.code==='ArrowLeft')sfRelease(-1);
    else if(e.code==='KeyD'||e.code==='ArrowRight')sfRelease(1);
  });

  PuzzleModes.register({
    id:'torqueangle', start:startTorqueangleRound, render:renderTorqueangle,
    tick:({dt})=>sfTick(Math.min(.04,dt/1000)),
    objective:()=>GameCatalog.get('torqueangle')?.objective,
    restartMessage:'Новая попытка: «Угол натяжения»',
    input:{horizontal:()=>{},vertical:()=>{}},
    attemptOpen:tryOpenTorqueangle
  });
})();
