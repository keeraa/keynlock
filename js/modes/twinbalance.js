...
function startTwinbalanceRound(){
  ...
  ptgDur=PTG_DUR_MAX;
  updateEconomyUI(); renderTwinbalance();
}
...
function tryOpenTwinbalance(){
  if(solved) return;
  registerMove();
  if(ptgCanLock()){
    solved=true; ptgV=[0,0];
    $lock.classList.add('win'); SFX.open(); renderTwinbalance();
...
function ptgTick(dt){
  if(mode!=='twinbalance'||solved) return;
  for(let i=0;i<2;i++){
...
document.getElementById('ptgConfirmBtn')?.addEventListener('click',()=>GameActions.attemptOpen({modeId:'twinbalance',source:'puzzle-control'}));
PuzzleModes.register({id:'twinbalance',start:startTwinbalanceRound,render:renderTwinbalance,tick:({dt})=>ptgTick(Math.min(.035,dt/1000)),objective:()=>GameCatalog.get('twinbalance')?.objective,restartMessage:'Новая попытка: «Двойное равновесие»',input:{horizontal:dir=>ptgHit(dir<0?0:1),vertical:()=>{}},actions:{primary:()=>GameActions.attemptOpen({modeId:'twinbalance',source:'keyboard'})},attemptOpen:tryOpenTwinbalance});
})();
