...
(() => {
  'use strict';
  const lessons={
    wharf:[['Задвижки','Сай: «Порядок скрыт. Запоминай удачные нажатия; ошибка сбрасывает цепочку. В первом заказе одна отмычка сломается по ходу обучения».','Первая задвижка поднялась, вторая сбросила цепочку. Как продолжить?',['Повторить удачную первую и попробовать другую','Нажимать вторую снова'],0]],
    hillsfar:[['Подбор ключа','Тик: «Сравни каждый выступ ключа с выемкой. Выбор безопасен; проверка скобами вставит ключ. Несовпадение сломает одну отмычку. На раунд — 30 секунд».','В выемке слева квадратный паз, справа треугольный. Какой ключ выбрать?',['Слева квадратный зубец, справа треугольный','Слева треугольный, справа квадратный'],0]],
    mass2:[['Парные узлы','Тик: «Наведи указатель, чтобы увидеть символ. На сенсорном экране удерживай узел. Найди четыре пары за 75 секунд; ошибка не тратит отмычки».','Первый узел — ◈. Какой открыть вторым?',['Ψ','◈','⊙'],1]],
    ...
    bioshock2:[['Остановка иглы','Тик: «Останавливай иглу в зелёной зоне. Нужно пройти пять этапов. Белая и красная зоны опасны для инструмента; красная может сбросить этапы. Не торопись: общего таймера здесь нет».','Когда нажимать остановку?',['На красном участке','Внутри зелёного участка','Между цветными участками'],1]],
  ...
  const help=document.createElement('button');help.id='missionLessonHelp';help.className='uiButton';help.textContent='?';help.setAttribute('aria-label','Объяснение механики');document.body.append(help);help.hidden=true;
