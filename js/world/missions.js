  ...
  // Position is the only thing authored per game; the label is read off the mode
  // tab so a renamed game cannot drift out of sync with its map node. Adding a
  // game means adding its tab and one line here.
  const MISSION_PLACES = [
    { mode: 'classic',   tab: 'tabClassic',   x: 28, y: 62 },
    { mode: 'target',    tab: 'tabTarget',    x: 22, y: 50 },
    { mode: 'line',      tab: 'tabLine',      x: 30, y: 38 },
    { mode: 'sequence',  tab: 'tabAlt2',      x: 38, y: 30 },
    { mode: 'special',   tab: 'tabSpecial',   x: 34, y: 47 },
    { mode: 'hillsfar',  tab: 'tabHillsfar',  x: 26, y: 28 },
    { mode: 'mass',      tab: 'tabMass',      x: 42, y: 24 },
    { mode: 'g1',        tab: 'tabG1',        x: 44, y: 44 },
    { mode: 'r2',        tab: 'tabR2',        x: 52, y: 40 },
    { mode: 'skyrim',    tab: 'tabSkyrim',    x: 44, y: 64 },
    { mode: 'anach',     tab: 'tabAnach',     x: 55, y: 63 },
    { mode: 'tension',   tab: 'tabTension',   x: 62, y: 33 },
    { mode: 'resonance', tab: 'tabResonance', x: 70, y: 28 },
    { mode: 'deduction', tab: 'tabDeduction', x: 78, y: 38 },
    { mode: 'composite', tab: 'tabComposite', x: 72, y: 52 },
    { mode: 'heatcold',  tab: 'tabHeatCold',  x: 80, y: 60 },
    { mode: 'drum',      tab: 'tabDrum',      x: 64, y: 72 },
    { mode: 'scope',     tab: 'tabScope',     x: 56, y: 80 }
  ];
  ...
  function missionLabel(place) {
    return document.querySelector(`#${place.tab}`)?.textContent?.trim() || place.mode;
  }
