/* Alchemy prototypes — extracted from the single-file demo.
   Everything lives inside this closure on purpose: the game loads classic
   scripts that share one lexical environment, and this file declares 231
   top-level names. Loose, two of them (`apply`, `shuffle`) already collide with
   the game, and every future name either side adds is another chance to break
   the page. Expose deliberately through window.Alchemy when wiring it in. */
(function(){
'use strict';
  ...
  function resolveLiquidHost(target){return typeof target==='string'?document.getElementById(target):target||null}
  ...
  const flasksEl=document.getElementById('flasks'), flaskNote=document.getElementById('flaskNote');
  ...
  document.getElementById('refillFlasks').onclick=()=>{flaskCount=3;lastBroken=-1;renderFlasks()};renderFlasks();
  ...
  let seq=[],seqPos=0;const sigWrap=document.getElementById('sigils'),dots=document.getElementById('seqDots'),seqTubeLayers=document.getElementById('seqTubeLayers');
  ...
  function updateSeq(){[...dots.children].forEach((d,i)=>d.classList.toggle('on',i<seqPos));document.getElementById('seqStatus').innerHTML=`<strong>${seqPos} / 4</strong>${seqPos===4?'эликсир собран':'этапов'}`}
  document.getElementById('seqReset').onclick=randomizeSequence;randomizeSequence();
  ...
  function renderDistill(now=performance.now()){
    ...
    document.getElementById('distillStatus').innerHTML=`<strong>${ok} / 5</strong>${ok===5?'цветов поймано':'цветов зафиксировано'}`
  }
  ...
  const recipeKeys=['1','2','3','4','5','6','7','8','9','Q','W','E'];let recipeTargets=[],recipeRequirements=[],recipeChoices=[],chosen=[];const recipeCluesEl=document.getElementById('recipeClues'),recipeTubeStack=document.getElementById('recipeTubeStack'),partWrap=document.getElementById('recipeParts');
  ...
  function checkRecipe(){if(chosen.length<4){document.getElementById('recipeStatus').innerHTML=`<strong>${chosen.length} / 4</strong>сначала добавь четыре компонента`;return}let ok=0;chosen.forEach((id,step)=>{const item=ingredientCatalog.find(x=>x.id===id);if(recipeMatches(item,step))ok++});if(ok===4){document.getElementById('recipeStatus').innerHTML='<strong>4 / 4</strong>формула верна'}else{breakFlask('Формула не соответствует рецепту');document.getElementById('recipeStatus').innerHTML=`<strong>${ok} / 4</strong>правильных этапа`}}
  function drawRecipe(rebuildTube=true){recipeCluesEl.innerHTML=recipeRequirements.map((props,i)=>`<div class="recipe-line-v8"><b>${i+1}. ${['Основа','Растворитель','Связка','Активатор'][i]}</b>${props.join(' + ')}</div>`).join('');if(rebuildTube){recipeTubeStack.innerHTML='';chosen.forEach((id,step)=>{const item=ingredientCatalog.find(x=>x.id===id);const layer=document.createElement('div');layer.className='formula-tube-layer-v8';layer.style.background=item.color;layer.style.setProperty('--layer-bottom-v92',`${step*25}%`);layer.innerHTML=`<i class="ti ${item.icon}"></i><span>${item.name}</span>`;recipeTubeStack.appendChild(layer)});applyTopMeniscusV96(recipeTubeStack,'.formula-tube-layer-v8');refreshPassiveBubbles(recipeTubeStack)}document.getElementById('recipeProgress').textContent=chosen.length?`${chosen.length} из 4 компонентов`:'пусто';partWrap.innerHTML='';recipeChoices.forEach((item,i)=>{const b=document.createElement('button');b.className='formula-part-v8'+(chosen.includes(item.id)?' used':'');b.innerHTML=`<span class="formula-key-v8">${recipeKeys[i]}</span><div class="ingredient-icon-v8"><i class="ti ${item.icon}"></i></div><div class="ingredient-name">${item.name}</div><div class="property-tags">${item.props.map(p=>`<span class="property-tag">${p}</span>`).join('')}</div>`;b.onclick=()=>chooseIngredientByIndex(i);partWrap.appendChild(b)});document.getElementById('recipeStatus').innerHTML=`<strong>${chosen.length} / 4</strong>${chosen.length===4?'можно проверять':'компонентов добавлено'}`}
  document.getElementById('recipeCheck').onclick=checkRecipe;document.getElementById('recipeReset').onclick=resetRecipe;newRecipe();
  ...
  function setStatusHtmlV98(id,html){const el=document.getElementById(id);if(el)el.innerHTML=html}
  ...
  document.getElementById('mixCheck').onclick=()=>{
  ...
  document.getElementById('mixReset').onclick=newMixRound;drawMix();animateSceneLiquidV98('mixCurrent',mixLevelTopV88(),{animate:false});
  ...
  document.getElementById('unknownCheck').onclick=()=>{
  ...
  document.getElementById('unknownReset').onclick=rerollUnknown;rerollUnknown();
  ...
  document.getElementById('sepCheck').onclick=()=>{
  ...
  document.getElementById('sepReset').onclick=resetSeparation;resetSeparation();


  const compoundDial=document.getElementById('compoundDial');
  const compoundPulseHand=document.getElementById('compoundPulseHand');

  const compoundZoneSvg=document.getElementById('compoundZoneSvg');
  const compoundTubeStack=document.getElementById('compoundTubeStack');
  ...
  const compoundTubeTargets=document.getElementById('compoundTubeTargets');
  ...
  function renderCompoundTube(){
    ...
    document.getElementById('compoundTubeLabel').textContent=
  ...
  function renderCompound(){
    ...
    renderCompoundZones();
    renderCompoundTube();
    const heatBtn=document.getElementById('compoundHeat');
    ...
    document.getElementById('compoundStatus').innerHTML=
    ...
    const h=document.getElementById('compoundHint');
    ...
    const impurityEl=document.getElementById('compoundImpurity');
    if(impurityEl){
  ...
  const compoundHeatBtn=document.getElementById('compoundHeat');
  ...
  function newEssenceRound(){essenceSelected=new Set();essenceCurrentTopV88=94;const ids=shuffle([0,1,2,3,4,5]).slice(0,3);essenceTarget=sumEssence(ids);renderEssence();const stack=document.querySelector('#essenceCurrentBars .essence-stack');if(stack){stack.dataset.liquidTopV88=String(essenceCurrentTopV88);animateStackClipLevelV88(stack,essenceCurrentTopV88,{animate:false})}}
  function toggleEssence(i){const from=essenceCurrentTopV88;if(essenceSelected.has(i))essenceSelected.delete(i);else if(essenceSelected.size<3)essenceSelected.add(i);const to=essenceSelected.size?78-(essenceSelected.size-1)*29:94;renderEssence();const stack=document.querySelector('#essenceCurrentBars .essence-stack');if(stack){stack.dataset.liquidTopV88=String(from);animateStackClipLevelV88(stack,to,{duration:1250,intensity:.75})}essenceCurrentTopV88=to}
  ...
  function renderEssence(){renderEssenceBars('essenceTargetBars',essenceTarget);renderEssenceBars('essenceCurrentBars',sumEssence([...essenceSelected]));const box=document.getElementById('essenceCards');box.innerHTML='';essenceLibrary.forEach((x,i)=>{const c=document.createElement('button');c.className='essence-card'+(essenceSelected.has(i)?' selected':'');c.innerHTML=`<span class="key">${i+1}</span><i class="ti ${x.icon}"></i><div class="proto-title">${x.name}</div><div class="essence-pips">${x.v.map((v,j)=>Array(v).fill(`<span class="essence-pip" style="background:${essenceColors[j]}"></span>`).join('')).join('')}</div>`;c.onclick=()=>toggleEssence(i);box.appendChild(c)});document.getElementById('essenceStatus').innerHTML=`<strong>${essenceSelected.size} / 3</strong>ингредиента выбрано`}
  function checkEssence(){const cur=sumEssence([...essenceSelected]),ok=essenceSelected.size===3&&cur.every((v,i)=>v===essenceTarget[i]);document.getElementById('essenceStatus').innerHTML=ok?'<strong>Баланс совпал</strong>зелье стабильно':'<strong>Баланс неверный</strong>соотношение не совпало';if(!ok)breakFlask('Неверный баланс эссенций')}
  document.getElementById('essenceCheck').onclick=checkEssence;document.getElementById('essenceReset').onclick=newEssenceRound;newEssenceRound();
  ...
  let layerReactionAnimatingV88=false;function applyLayerReagent(key){if(!layerState.length||layerReactionAnimatingV88)return;const top=layerState[0];let action='none',next=null;if(key==='solvent'&&top==='oil')action='remove';else if(key==='heat'&&top==='water')action='remove';else if(key==='heat'&&top==='ice'){action='transform';next='water'}else if(key==='salt'&&top==='sludge'){action='transform';next='crystal'}else if(key==='cold'&&top==='water'){action='transform';next='ice'}if(action==='none'){breakFlask('Реагент не взаимодействует с верхним слоем');return}const stack=document.getElementById('layerReactStack'),topEl=stack.querySelector('.layer-react-layer');if(action==='remove'&&topEl){layerReactionAnimatingV88=true;animateLayerRemovalV88(topEl,640,()=>{layerState.shift();renderLayerGame();layerReactionAnimatingV88=false})}else{layerState[0]=next;renderLayerGame();waveLiquid('layerReactStack',.65)}}
  function renderLayerGame(){const stack=document.getElementById('layerReactStack');stack.innerHTML='';layerState.forEach(t=>{const d=document.createElement('div');d.className='layer-react-layer';d.style.background=layerTypes[t].color;d.textContent=layerTypes[t].name;stack.appendChild(d)});const top=layerState[0];const won=layerState.length===1&&top==='crystal';document.getElementById('layerTopLabel').textContent=won?'получен чистый кристалл':top?`верхний слой: ${layerTypes[top].name}`:'реактор пуст';document.getElementById('layerStatus').innerHTML=won?'<strong>Кристалл получен</strong>реакция завершена':'<strong>Цель: кристалл</strong>реагируй с верхним слоем';const box=document.getElementById('layerReagents');box.innerHTML='';layerReagentDefs.forEach((r,i)=>{const b=document.createElement('button');b.className='layer-reagent';b.innerHTML=`<i class="ti ${r.icon}"></i><strong>${r.name}</strong><div class="proto-sub">клавиша ${i+1}</div>`;b.onclick=()=>applyLayerReagent(r.key);box.appendChild(b)})}
  document.getElementById('layerReset').onclick=resetLayerGame;resetLayerGame();
  ...
  function renderBranch(){
    document.getElementById('branchTargetName').textContent=branchElixirName(branchTarget);
    const tags=document.getElementById('branchTargetTags');tags.innerHTML='';
    ...
    document.getElementById('branchCurrentName').textContent=branchPath.length?branchMeta(branchPath.at(-1)).name:'Базовый настой';
    const steps=document.getElementById('branchSteps');steps.innerHTML='';for(let i=0;i<3;i++){const s=document.createElement('span');s.className='branch-step'+(i<branchPath.length?' done':'');steps.appendChild(s)}
    const box=document.getElementById('branchReagents');box.innerHTML='';
    ...
    if(stage===3){const ok=branchPath.every((v,i)=>v===branchTarget[i]);document.getElementById('branchStatus').innerHTML=ok?'<strong>Эликсир готов</strong>ветка собрана верно':'<strong>Получился другой эликсир</strong>целевая ветка не совпала';if(!ok){breakFlask('Сварен неверный эликсир');branchPath=[];setTimeout(renderBranch,10)}}
    else document.getElementById('branchStatus').innerHTML=`<strong>Шаг ${stage+1} / 3</strong>${['выбери основу','выбери эффект','выбери стабилизатор'][stage]}`
  }
  document.getElementById('branchReset').onclick=newBranchTarget;newBranchTarget();

  function activeGameName(){const s=document.querySelector('.scene.active');return s?s.dataset.name:''}
  document.addEventListener('keydown',e=>{if(e.ctrlKey||e.metaKey||e.altKey)return;const tag=(e.target&&e.target.tagName||'').toLowerCase();if(tag==='input'||tag==='textarea'||tag==='select')return;const game=activeGameName();let handled=true;if(game==='Порядок'){if(['1','2','3','4'].includes(e.key))chooseSequence(Number(e.key)-1);else if(e.code==='KeyR')randomizeSequence();else handled=false}else if(game==='Перегонка'){if(['1','2','3','4','5'].includes(e.key))captureDistill(Number(e.key)-1);else handled=false}else if(game==='Формула'){const map={'1':0,'2':1,'3':2,'4':3,'5':4,'6':5,'7':6,'8':7,'9':8,'q':9,'w':10,'e':11},k=e.key.toLowerCase();if(k in map)chooseIngredientByIndex(map[k]);else if(e.key==='Enter')checkRecipe();else if(e.code==='KeyR')resetRecipe();else handled=false}else if(game==='Смешение'){if(e.key==='1'){mixAmts[0]++;drawMix();setSingleLiquidLevelV88('mixCurrent',mixLevelTopV88(),{duration:1350,intensity:1})}else if(e.key==='2'){mixAmts[1]++;drawMix();setSingleLiquidLevelV88('mixCurrent',mixLevelTopV88(),{duration:1350,intensity:1})}else if(e.key==='3'){mixAmts[2]++;drawMix();setSingleLiquidLevelV88('mixCurrent',mixLevelTopV88(),{duration:1350,intensity:1})}else if(e.key==='4'){mixAmts[3]++;drawMix();setSingleLiquidLevelV88('mixCurrent',mixLevelTopV88(),{duration:1350,intensity:1})}else if(e.key==='Enter')document.getElementById('mixCheck').click();else if(e.code==='KeyR')document.getElementById('mixReset').click();else handled=false}else if(game==='Концентрации'){if(e.key==='1'){unkAmts[0]++;drawUnknown();setSingleLiquidLevelV88('unknownCurrent',unknownLevelTopV88(),{duration:1350,intensity:1})}else if(e.key==='2'){unkAmts[1]++;drawUnknown();setSingleLiquidLevelV88('unknownCurrent',unknownLevelTopV88(),{duration:1350,intensity:1})}else if(e.key==='3'){unkAmts[2]++;drawUnknown();setSingleLiquidLevelV88('unknownCurrent',unknownLevelTopV88(),{duration:1350,intensity:1})}else if(e.key==='4'){unkAmts[3]++;drawUnknown();setSingleLiquidLevelV88('unknownCurrent',unknownLevelTopV88(),{duration:1350,intensity:1})}else if(e.key==='Enter')document.getElementById('unknownCheck').click();else if(e.code==='KeyR')document.getElementById('unknownReset').click();else handled=false}else if(game==='Разделение'){if(['1','2','3','4'].includes(e.key)){const b=document.querySelectorAll('#sepControls .small-btn')[Number(e.key)-1];if(b)b.click()}else if(e.key==='Enter')document.getElementById('sepCheck').click();else if(e.code==='KeyR')document.getElementById('sepReset').click();else handled=false}else if(game==='Давление + осаждение'){if(e.code==='Space'){setCompoundHeat(true)}else if(e.code==='KeyR')resetCompound();else handled=false}else if(game==='Баланс эссенций'){if(['1','2','3','4','5','6'].includes(e.key))toggleEssence(Number(e.key)-1);else if(e.key==='Enter')checkEssence();else if(e.code==='KeyR')newEssenceRound();else handled=false}else if(game==='Реакции слоёв'){if(['1','2','3','4'].includes(e.key))applyLayerReagent(layerReagentDefs[Number(e.key)-1].key);else if(e.code==='KeyR')resetLayerGame();else handled=false}else if(game==='Ветвящийся рецепт'){if(['1','2','3','4','5','6','7','8','9'].includes(e.key))chooseBranchReagent(branchReagentDefs[Number(e.key)-1].key);else if(e.code==='KeyR')newBranchTarget();else handled=false}else handled=false;if(handled)e.preventDefault()});
